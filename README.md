...
4. **Stage 4 — Causal Inference Suite**: Attributes causation using econometric estimators operating on CUPED-adjusted outcome data:
   ...
   * Synthetic Control Method (cvxpy weight optimization)
...
* **JEPA & Reinforcement Learning Connection**: The explainable causal counterfactuals generated by Stage 4 act as ground-truth target structures to train future latent-space models (like Meta's Joint Embedding Predictive Architecture — JEPA). Additionally, Stage 2's sequential monitoring mirrors Thompson Sampling multi-armed bandits, while Stage 4 BSTS state-space models correspond mathematically to POMDP belief-state tracking in RL agents. See [omnistats/README.md](file:///c:/Users/mrdat/PycharmProjects/pan-theory/omnistats/README.md) for details.
