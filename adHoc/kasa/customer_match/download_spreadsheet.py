...
def main():
    ...
    FILE_ID = '1--copNwH0SNuv0rTmwpiCzVvJ1K_D82QMvku0HPfPtQ'
    
    # MIME type for Microsoft Excel (.xlsx)
    EXPORT_MIME_TYPE = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    
    # Target local filename
    OUTPUT_FILENAME = 'vinpearl_data_model.xlsx'

    try:
        creds = get_credentials()
        service = build('drive', 'v3', credentials=creds)

        print(f"Exporting Google Spreadsheet (ID: {FILE_ID}) as {OUTPUT_FILENAME}...")
        ...
        print(f"\nSuccess! Spreadsheet downloaded to {os.path.abspath(OUTPUT_FILENAME)}")
