...
def main():
    # --- CONFIGURATION ---
    ACCOUNT_ID = '231663578' # Defaulting to Kasatria VN for this template
    PARENT = f"accounts/{ACCOUNT_ID}"
    
    USER_EMAIL = 'udemysharedcontent@gmail.com'
    
    # Predefined role for Viewer
    ...
    try:
        ...
        print(f"GRANTING ACCOUNT VIEWER ACCESS...")
        print(f"Target Account: {PARENT}")
        ...
        print("\nSuccess! Account Viewer access granted.")
        print(f"Access Binding: {result.get('name')}")
        print(f"User: {result.get('user')}")

    except Exception as e:
        print(f"Execution Error: {e}")
        print("\nTIP: Ensure the Service Account has 'Admin' level access to the ACCOUNT to manage users.")
