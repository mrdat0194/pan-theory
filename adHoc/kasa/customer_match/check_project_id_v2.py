...
# Load API keys from environment variable GOOGLE_API_KEYS (comma-separated)
keys = [
...
def check_project_details(k):
    ...
    try:
        ...
        if match:
            return f"Identifier: {match.group(1)}"

        # Search for project number
        match_n = re.search(r"project ([\w.-]+)", msg)
        if match_n:
            return f"Number/ID: {match_n.group(1)}"
...
if __name__ == "__main__":
    if not keys:
        print("Warning: No API keys found in GOOGLE_API_KEYS environment variable.")
