...
*   **[fft_polynomial.py](file:///C:/Users/mrdat/PycharmProjects/pan-theory/datastructure/fft_polynomial.py)**: Educational implementation of the Cooley-Tukey Fast Fourier Transform (FFT) and Inverse FFT (IFFT) from scratch using pure Python. Features step-by-step polynomial evaluation, interpolation, and $O(n \log n)$ polynomial multiplication.
    *   *Note:* This serves as the math-transparent, educational counterpart to the pre-compiled `np.fft` routines used in the repository's competitive coding scripts (e.g. the `T3sum` pairwise sum frequency counter in [submit.py](file:///C:/Users/mrdat/PycharmProjects/pan-theory/submit.py#L308)).
